...
import { updateOrderAdminData } from "../Thunk/adminThunk";
import {
  Grid,
  Typography,
  Card,
...
import { getAllVendorOrderData } from "../Thunk/orderThunk";

function VendorOrders() {
  const dispatch = useDispatch();
  const { orderVendorData } = useSelector((state) => state.order);
  console.log(orderVendorData, "_____________________");
  const [value, setValue] = useState(0);
  const [filter, setFilter] = useState("");
  const [loading, setLoading] = useState(false);
  useEffect(() => {
    dispatch(getAllVendorOrderData(filter));
  }, [filter, dispatch]);
    ...
    <>
      <Container maxWidth={false} disableGutters sx={{ p: 2 }}>
        ...
        {loading ? (
          ...
          >
            <Box>
              {orderVendorData?.length === 0 ? (
                <Typography variant="body1">No orders found.</Typography>
              ) : (
                orderVendorData.map((order) => (
                        ...
                        >
                          <Button
                            onClick={async () => {
                              ...
                              try {
                                await dispatch(
                                  updateOrderAdminData(order?._id)
                                );
                              } catch (error) {
